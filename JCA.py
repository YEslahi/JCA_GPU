...
class JCA:
    ...
    def run(self, train_R, vali_R, price_R):
        print("in JCA run")
        self.train_R = train_R
        self.vali_R = vali_R
    ...
    def train_model(self, itr):
        ...
        for i in xrange(self.num_batch_U):  # iterate each batch
            ...
            for j in xrange(self.num_batch_I):
                ...
                print(" in JCA line 228, check the input and output size of minibatch.")
        ...
        if itr % self.display_step == 0:
            ...
            if itr % 1 == 0:
                if self.base == 'i':
                    ...
                    [precision, recall, f_score, NDCG] = utility.test_model_all(Decoder, self.vali_R, self.train_R, self.price_R)
    ...
    def make_records(self):  # record all the results' details into files
        ...
        _, Decoder = self.sess.run([self.cost, self.Decoder],
                                   feed_dict={
                                        ...
                                        self.col_idx: np.reshape(xrange(self.num_cols), (self.num_cols, 1))})
        if self.base == 'i':
            [precision, recall, f_score, NDCG] = utility.test_model_all(Decoder.T, self.vali_R.T, self.train_R.T, self.price_R.T, 'save_intermediate_results')
        else:
            [precision, recall, f_score, NDCG] = utility.test_model_all(Decoder, self.vali_R, self.train_R, self.price_R, 'save_intermediate_results')
