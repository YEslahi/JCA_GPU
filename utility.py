...
def user_precision_recall_ndcg(new_user_prediction, test, products_prices=None):
    ...
    count_1, count_5, count_10, count_15 = 0.0, 0.0, 0.0, 0.0
    revenue_1, revenue_5, revenue_10, revenue_15 = 0.0, 0.0, 0.0, 0.0
    for i in xrange(15):
    ...
    return np.array([precision_1, precision_5, precision_10, precision_15]),\
           ...
           np.array([revenue_1, revenue_5, revenue_10, revenue_15]),
...
def test_model_all(prediction, test_mask, train_mask, price_mask, store_intermediate='no_store'):
    ...
    revenue_1, revenue_5, revenue_10, revenue_15 = 0.0000, 0.0000, 0.0000, 0.0000
    ...
    revenue = np.array([revenue_1, revenue_5, revenue_10, revenue_15])
    print("in test_model_all...")
    ...
    print("in test model all...")
    print("check the prediction vector and test vector")
    user_num = prediction.shape[0]
    prod_train_cnt = [int(cnt) if cnt < 5 else 5 for cnt in train_mask.sum(axis=1)]
    metrics_by_product_count = {'revenue':{}, 'precision':{}}
    # Baslerall_results_df = pd.DataFrame(columns=['user', 'top5_predictions', 'product_in_GT'])
    ...
    for u in range(user_num):  # iterate each user
        ...
        u_prod_train_cnt = prod_train_cnt[u]
        ...
        if not len(u_test) == 0:
            precision_u, recall_u, ndcg_u, revenue_u = user_precision_recall_ndcg(top15, u_test, top15_price)
            try:
                metrics_by_product_count['revenue']['{}_product'.format(u_prod_train_cnt)] += revenue_u
            except KeyError:
                metrics_by_product_count['revenue']['{}_product'.format(u_prod_train_cnt)] = revenue_u

            try:
                metrics_by_product_count['precision']['{}_product'.format(u_prod_train_cnt)] += precision_u
            except KeyError:
                metrics_by_product_count['precision']['{}_product'.format(u_prod_train_cnt)] = precision_u
    ...
    print("evaluation and storing data took:", b-a)
    # Baslerall_results_df.to_csv('./Basler_fromdf_res2.csv', index=False)
    ...
    print ('ndcg_1     \t[%.7f],\t||\t ndcg_5     \t[%.7f],\t||\t ndcg_10     \t[%.7f],\t||\t ndcg_15     \t[%.7f]' \
          % (ndcg[0],
             ...
             ndcg[2],
             ndcg[3]))
    print ('revenue_1     \t[%.7f],\t||\t revenue_5     \t[%.7f],\t||\t revenue_10     \t[%.7f],\t||\t revenue_15     \t[%.7f]' \
          % (revenue[0],
             revenue[1],
             revenue[2],
             revenue[3]))
    print('metrics_by_product_count["revenue"]:', metrics_by_product_count['revenue'])
    print('metrics_by_product_count["precision"]:', metrics_by_product_count['precision'])
...
def test_model_batch(prediction, test_mask, train_mask):
    ...
    for u in range(user_num):  # iterate each user
        ...
        # calculate the metrics
        if not len(u_test) == 0:
            precision_u, recall_u, ndcg_u, revenue_u = user_precision_recall_ndcg(top15, u_test)
            precision += precision_u
            recall += recall_u
            ndcg += ndcg_u
        else:
